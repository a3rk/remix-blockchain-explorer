...
main(int ac, const char* av[])
{
    ...
    rmxeg::CmdLineOptions opts {ac, av};
    ...
    if (!rmxeg::get_blockchain_path(bc_path_opt, blockchain_path, testnet))
    {
    ...
    rmxeg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!rmxeg::init_blockchain(blockchain_path.string(),
                               mcore, core_storage))
    {
    ...
    if (enable_emission_monitor == true)
    {
        ...
        rmxeg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        rmxeg::CurrentBlockchainStatus::testnet
                = testnet;
        rmxeg::CurrentBlockchainStatus::deamon_url
                = deamon_url;
        rmxeg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        rmxeg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    rmxeg::MempoolStatus::blockchain_path
            = blockchain_path;
    rmxeg::MempoolStatus::testnet
            = testnet;
    rmxeg::MempoolStatus::deamon_url
            = deamon_url;
    rmxeg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    rmxeg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    rmxeg::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    rmxeg::page rmxblocks(&mcore,
                          core_storage,
                          deamon_url,
                          testnet,
                          enable_pusher,
    ...
    ([&](const crow::request& req) {
        return crow::response(rmxblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return rmxblocks.index2(page_no);
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](const crow::request& req, size_t block_height) {
        return crow::response(rmxblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](const crow::request& req, string block_hash) {
        return crow::response(rmxblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](const crow::request& req, string tx_hash) {
        return crow::response(rmxblocks.show_tx(remove_bad_chars(tx_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>/<uint>")
    ([&](string tx_hash, uint16_t with_ring_signatures)
     {
        return rmxblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures);
    });
    ...
    ([&](const crow::request& req)
     {
        ...
        map<std::string, std::string> post_body
                = rmxeg::parse_crow_post_data(req.body);

        if (post_body.count("rmx_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("rmx address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string rmx_address = remove_bad_chars(post_body["rmx_address"]);
        ...
        return rmxblocks.show_my_outputs(tx_hash, rmx_address,
                                         viewkey, raw_tx_data,
                                         domain);
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string rmx_address, string viewkey)
     {

        string domain = get_domain(req);

        return rmxblocks.show_my_outputs(remove_bad_chars(tx_hash),
                                         remove_bad_chars(rmx_address),
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            if (post_body.count("rmxaddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("rmx address, tx private key or "
            ...
            string rmx_address = remove_bad_chars(post_body["rmxaddress"]);
            ...
            return rmxblocks.show_prove(tx_hash,
                                        rmx_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain);
    ...
    ([&](const crow::request& req, string tx_hash,
         string rmx_address, string tx_prv_key) {

        string domain = get_domain(req);

        return rmxblocks.show_prove(remove_bad_chars(tx_hash),
                                    remove_bad_chars(rmx_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&](const crow::request& req) {
            ...
            if (action == "check")
                return rmxblocks.show_checkrawtx(raw_tx_data, action);
            else if (action == "push")
                return rmxblocks.show_pushrawtx(raw_tx_data, action);
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&](const crow::request& req) {
            return rmxblocks.show_rawkeyimgs();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return rmxblocks.show_checkrawkeyimgs(raw_data, viewkey);
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&](const crow::request& req) {
            return rmxblocks.show_rawoutputkeys();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return rmxblocks.show_checkcheckrawoutput(raw_data, viewkey);
        });
    }
    ...
    ([&](const crow::request& req) {
        return rmxblocks.search(remove_bad_chars(string(req.url_params.get("value"))));
    });

    CROW_ROUTE(app, "/mempool")
    ([&](const crow::request& req) {
        return rmxblocks.mempool(true);
    });
...
//    });
    ...
    if (enable_js)
    {
        ...
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("jquery.min.js");
        });

        CROW_ROUTE(app, "/js/crc32.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("crc32.js");
        });

        CROW_ROUTE(app, "/js/biginteger.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("biginteger.js");
        });

        CROW_ROUTE(app, "/js/crypto.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("crypto.js");
        });

        CROW_ROUTE(app, "/js/config.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("config.js");
        });

        CROW_ROUTE(app, "/js/nacl-fast-cn.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("nacl-fast-cn.js");
        });

        CROW_ROUTE(app, "/js/base58.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("base58.js");
        });

        CROW_ROUTE(app, "/js/cn_util.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("cn_util.js");
        });

        CROW_ROUTE(app, "/js/sha3.js")
        ([&](const crow::request& req) {
            return rmxblocks.get_js_file("sha3.js");
        });
        ...
        ([&](const crow::request& req) {
            ...
            return rmxblocks.get_js_file("all_in_one.js");
        });
    ...
    if (enable_json_api)
    {
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](const crow::request &req, string search_value) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_networkinfo()};
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_outputs(
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_outputsblocks(
        ...
        ([&](const crow::request &req) {
            ...
            myrmx::jsonresponse r{rmxblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return rmxblocks.index2(page_no, refresh_page);
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        rmxeg::CurrentBlockchainStatus::m_thread.interrupt();
        rmxeg::CurrentBlockchainStatus::m_thread.join();
    ...
    rmxeg::MempoolStatus::m_thread.interrupt();
    rmxeg::MempoolStatus::m_thread.join();
