...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        last_tx.fee_str         = rmxeg::rmx_amount_to_str(_tx_info.fee, "{:0.3f}", false);
        last_tx.rmx_inputs_str  = rmxeg::rmx_amount_to_str(last_tx.sum_inputs , "{:0.3f}");
        last_tx.rmx_outputs_str = rmxeg::rmx_amount_to_str(last_tx.sum_outputs, "{:0.3f}");
        last_tx.timestamp_str   = rmxeg::timestamp_to_str_gm(_tx_info.receive_time);
...
bf::path MempoolStatus::blockchain_path {"/home/mwo/.bitremix/lmdb"};
...
rmxeg::MicroCore*  MempoolStatus::mcore {nullptr};
