...
/**
* Parse remix address in a string form into
...
/**
* Return string representation of remix address
*/
string
...
{
    // default path to remix folder
    // on linux this is /home/<username>/.bitremix
    string default_remix_dir = tools::get_default_data_dir();

    if (testnet)
        default_remix_dir += "/testnet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_remix_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = rmxeg::get_default_lmdb_folder(testnet);
    ...
    blockchain_path = rmxeg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_rmx {0, 0};
    ...
    {
        sum_rmx.first += vout["amount"].get<uint64_t>();
        ++sum_rmx.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_rmx {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t rmx_outputs       {0};
    uint64_t rmx_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        rmx_inputs += tx_in_to_key.amount;
    ...
    return {rmx_outputs, rmx_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        rmx_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {rmx_outputs, rmx_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_rmx.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_rmx.second;
    }
...
{
    array<uint64_t, 2> sum_rmx;

    sum_rmx[0] = sum_money_in_inputs(tx);
    sum_rmx[1] = sum_money_in_outputs(tx);
