...
#ifndef CROWRMX_RPCCALLS_H
#define CROWRMX_RPCCALLS_H
...
{
// declare struct in remix's cryptonote namespace.
// remix should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Remix deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Remix deamon at "
                 << deamon_url << endl;
            return false;
        }
